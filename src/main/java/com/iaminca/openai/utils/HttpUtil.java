...
public class HttpUtil {
    ...
    public static void main(String[] args) {
        try {
            String json = "   {\n" +
                    "  \"date\": \"2023-05-31T10:30:00\",\n" +
                    "  \"date_gmt\": \"2023-05-31T07:30:00\",\n" +
                    "  \"slug\": \"my-post\",\n" +
                    "  \"status\": \"publish\",\n" +
                    "  \"password\": \"\",\n" +
                    "  \"title\": \"2024海南夏天的旅游攻略\",\n" +
...
//                    "  \"author\": 1,\n" +
            ...
            String sendPostJson = sendPostJson("https://wp.martian.tk/wp-json/wp/v2/posts", json, headers);
     ...
     * @return
     */
    public static byte[] sendPostByte(String url, String json, Map<String, Object> headers) {
        byte[] result = null;
        try {
            HttpPost httpPost = new HttpPost(url);
            setHeaders(headers, httpPost);
            ...
            HttpResponse response = httpClient.execute(httpPost);
            HttpEntity responseData = response.getEntity();
            result = EntityUtils.toByteArray(responseData);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return result;
